...
if (adminBotToken) {
    ...
    setTimeout(() => {
        try {
            // НЕ устанавливаем отдельный порт - используем интегрированную админку
            // На Railway все должно работать через один порт
            
            // Проверяем ID админов
            const adminIds = process.env.ADMIN_IDS;
            if (!adminIds) {
                console.log('⚠️  ADMIN_IDS не установлено - админ-бот не запустится');
                console.log('💡 Добавьте ADMIN_IDS=your_telegram_id в переменные Railway');
                return;
            }
            
            // Запускаем только Telegram бота (веб-админка уже интегрирована в основной сервер)
            require('./admin-bot.js');
            
            console.log('✅ Оба бота запущены успешно!');
            console.log('🌐 Админка доступна по адресу: /admin/');
        } catch (error) {
            console.error('❌ Ошибка запуска админ-бота:', error.message);
